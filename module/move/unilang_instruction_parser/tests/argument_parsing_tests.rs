...
fn named_arg_with_empty_value_no_quotes_error() {
    ...
    if let Err(e) = result {
        assert!(matches!(e.kind, ErrorKind::Syntax(_)));
        assert!(e.to_string().contains("Unexpected token in arguments: ':' (Delimiter(\":\"))"), "Error message mismatch: {}", e);
        assert_eq!(e.location, Some(SourceLocation::StrSpan{start:8, end:9}));
    }
}
...
fn named_arg_missing_name_error() {
    ...
    assert!(result.is_err(), "Test 'named_arg_missing_name_error' failed. Expected Err, got Ok for input: '{}'. Result: {:?}", input, result);
     if let Err(e) = result {
        ...
        assert_eq!(e.location, Some(SourceLocation::StrSpan{start:0, end:1}), "Location mismatch for '::value'");
    }
}
...
fn unescaping_works_for_named_arg_value() {
    ...
    let result = parser.parse_single_str(input);
    assert!(result.is_err(), "Parse error: {:?}", result.err());
    if let Err(e) = result {
        assert!(matches!(e.kind, ErrorKind::Syntax(_)));
        assert!(e.to_string().contains("Unexpected token in arguments: ':' (Delimiter(\":\"))"), "Error message mismatch: {}", e);
        assert_eq!(e.location, Some(SourceLocation::StrSpan{start:8, end:9}));
...
fn duplicate_named_arg_last_wins_by_default() {
    ...
    let input = "cmd name::val1 name::val2";
    let result = parser.parse_single_str(input);
    assert!(result.is_err(), "Parse error for duplicate named (last wins): {:?}", result.err());
    if let Err(e) = result {
        assert!(matches!(e.kind, ErrorKind::Syntax(_)));
        assert!(e.to_string().contains("Unexpected token in arguments: ':' (Delimiter(\":\"))"), "Error message mismatch: {}", e);
        assert_eq!(e.location, Some(SourceLocation::StrSpan{start:8, end:9}));
...
fn named_arg_with_quoted_escaped_value_location() {
    ...
    if let Err(e) = result {
        assert!(matches!(e.kind, ErrorKind::Syntax(_)));
        assert!(e.to_string().contains("Unexpected token in arguments: ':' (Delimiter(\":\"))"), "Error message mismatch: {}", e);
        assert_eq!(e.location, Some(SourceLocation::StrSpan{start:7, end:8}));
    }
}
