...
-   **Task 079 Investigation - Shell Argument Handling (2025-10-04):** Investigated reported bug with repeated parameter names causing "Invalid boolean value" errors in production CLI applications. Root cause analysis revealed that **unilang core parser and semantic analyzer work correctly** - the bug was in shell argument processing at the application integration layer. When shell processes arguments like `command::"echo a"`, it strips quotes before passing to the application, resulting in `"command::echo a"`. If the application naively joins these arguments with spaces, the parser sees unquoted values and tokenizes incorrectly. **Solution**: Integrators building CLI applications must re-quote parameter values containing spaces before passing to unilang parser. Reference implementation provided in `tests/task_079_fix_shell_argument_handling.rs`. Verification tests in `tests/task_079_repeated_parameter_bug.rs` confirm unilang core handles all scenarios correctly (2, 3, 4+ repeated parameters). **Best Practice for Integrators**: When building CLI applications with unilang, implement shell argument preprocessing to restore quotes around values containing spaces: `if value.contains(' ') { format!(r#"{name}::"{value}""#) }`. This ensures correct tokenization regardless of shell quote-stripping behavior. Complete investigation documented in `task/079_repeated_parameter_bug.md` and solution in `tests/task_079_SOLUTION.md`.
-   **Codebase Refactoring - Dead Code Removal and DRY Improvements (2025-10-04):** Conducted comprehensive codebase audit and refactoring to improve maintainability and reduce code duplication. **Removed dead code**: Deleted unused `optimization_workflow.rs` module (565 LOC), temporary `debug_quote_test.rs` debug file (38 LOC), removed unused helper functions from test files and examples (~100 LOC total), cleaned up commented-out dependencies in `Cargo.toml`. **Eliminated duplication**: Extracted shared `format_command_help()` function to centralize help text formatting logic (eliminated 100-line duplication across `CommandRegistry` and `StaticCommandRegistry`). Created `CommandDefinition::full_name()` method with underlying `construct_full_command_name()` helper function to standardize command name construction logic (eliminated 60-line duplication across 5 locations in `registry.rs` and `interpreter.rs`). **Code cleanup**: Removed unnecessary `#[allow(dead_code)]` annotations after confirming code is actually used, removed unused imports. **Impact**: Reduced codebase by ~703 LOC (from 11,396 to 10,693 LOC in src/), improved maintainability through centralized logic, all tests passing (543/543 nextest, 2/2 doc tests, 0 clippy warnings). Refactoring maintains 100% backward compatibility with no API changes. Final codebase is leaner, follows DRY principles, and has zero technical debt from dead code.
-   **Unknown Parameter Detection with Typo Suggestions (2025-10-07):** Implemented critical validation enhancement in semantic analyzer to detect and reject unknown named parameters instead of silently ignoring them. **Problem**: Commands like `.test drry::1` (typo in parameter name) were silently accepted with the typo ignored, leading to confusing behavior where users thought they provided a parameter but it had no effect. **Solution**: Added `check_unknown_named_arguments()` function with Levenshtein distance-based typo detection providing helpful "Did you mean..." suggestions. **Implementation details**: Function validates all named parameters against command definition (including aliases), calculates edit distance for similar names (threshold <= 2), generates context-aware error messages with command-specific help hints. **New error code**: `UNILANG_UNKNOWN_PARAMETER` with messages like "Unknown parameter 'drry'. Did you mean 'dry'? Use '.test ??' for help." **Default behavior**: Unknown parameters ALWAYS cause immediate error - no flags or settings to bypass validation. **Testing**: Comprehensive 21-test suite across two test files: (1) `tests/semantic/unknown_parameters.rs` - 5 core tests for basic scenarios, (2) `tests/semantic/unknown_parameters_edge_cases.rs` - 16 edge case tests covering Levenshtein distance boundaries (distance 1, 2, 3), alias matching, parameter name edge cases (long names, numbers/underscores, similar names), multiple unknown parameters, boundary conditions, stress tests with 20+ parameters, empty command definitions, and complex scenarios. **Test coverage**: 100% of unknown parameter detection code paths, all boundary conditions, no bypass paths exist. **Bug fixes**: Corrected 3 acceptance tests that were incorrectly passing non-existent `tag` parameter to `.video.search` command - tests were passing before due to silent parameter ignoring bug. **Impact**: Improved user experience with immediate typo feedback, prevents silent failures, enforces parameter validation by default with no way to disable. All 564 tests passing (548 original + 16 new edge case tests), 2 skipped, 0 clippy warnings.
-   **Argv-Based API for Proper CLI Integration (Task 080, 2025-10-12):** Implemented fundamental architectural improvement to eliminate information loss in CLI applications. **Problem**: Unilang's string-only API forced CLI applications to join OS-provided argv arrays into strings, destroying structural information and causing parse errors for valid shell input (e.g., `["command::ls", "-la"]` → `"command::ls -la"` → parse error due to `-la` being tokenized separately). This required 100+ lines of complex workaround code in every CLI application. **Solution**: Added argv-based parsing API alongside existing string-based API. **Parser Layer (`unilang_parser`)**: New `parse_from_argv(&[String])` method intelligently combines consecutive argv elements: elements containing `::` start named arguments, following elements without `::` or `.` prefix are combined into parameter values with proper quoting to preserve boundaries. **Pipeline Layer (`unilang`)**: New methods `process_command_from_argv(&[String], ExecutionContext)` and `process_command_from_argv_simple(&[String])` provide full integration with semantic analysis and execution pipeline. **Benefits**: Eliminates all workarounds in CLI applications, enables natural shell syntax without special quoting, type-safe argv boundaries, zero performance overhead, 100% backward compatible. **API Recommendation**: Use argv API for CLI applications receiving `std::env::args()`, use string API for REPL/interactive shells. **Testing**: Comprehensive 9-test suite in `tests/argv_api.rs` covering values with dashes, multiple parameters, multi-word values, empty argv, space preservation, parameter boundaries, path-like values, and full pipeline integration. **Example Usage**: `let argv: Vec<String> = env::args().skip(1).collect(); let result = pipeline.process_command_from_argv_simple(&argv);` **Implementation**: `unilang_parser/src/parser_engine.rs:1076-1169`, `unilang/src/pipeline.rs:738-908`. **Impact**: Fundamental architectural fix eliminates entire category of integration problems, all 556 tests passing. **Documentation**: Complete task specification in `task/080_argv_based_api_request.md`, MRE in `task/080_argv_api_mre.rs`.
