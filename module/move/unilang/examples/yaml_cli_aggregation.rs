...
fn demonstrate_static_aggregation()
{
  ...
  println!( "Available static commands:" );
...
{
  /// Create registry from static commands (helper method for demo)
  fn from_static_commands( static_commands: &phf::Map< &'static str, &'static unilang::static_data::StaticCommandDefinition > ) -> Self
  {
    #[ allow( deprecated ) ]
    let mut registry = CommandRegistry::new();

    // Convert static commands to dynamic for demonstration
    for ( name, static_cmd ) in static_commands.entries()
    {
      let cmd_def: CommandDefinition = (*static_cmd).into();

      let routine = Box::new( move |_cmd: VerifiedCommand, _ctx: ExecutionContext|
      {
        Ok( OutputData
        {
          content: format!( "Executed {} (from static aggregation)", name ),
          format: "text".to_string(),
        })
      });

      #[ allow( deprecated ) ]
      if let Err( e ) = registry.command_add_runtime( &cmd_def, routine )
      {
