...
//! and demonstrates the performance benefits of PHF-based lookups.
//!
...
use unilang::static_data::{ StaticCommandMap, StaticCommandDefinition };
...
/// Static PHF map for performance testing
static MOCK_STATIC_MAP: StaticCommandMap = phf::phf_map!
{
...
fn setup_test_registries() -> Result< ( StaticCommandRegistry, CommandRegistry ), Box< dyn std::error::Error > >
{
  ...
  // Note: In real usage, this would be a compile-time generated PHF map
  // Use the static PHF map defined at module level
  let static_registry = StaticCommandRegistry::from_phf( &MOCK_STATIC_MAP );
