...
{
  let f = | x : &nelder_mead::Point | ( x.coords[ 0 ] * x.coords[ 0 ] ).into_inner();
...
fn sin_cos() -> Result< (), nelder_mead::Error >
{
  ...
  assert!( ( -1.5808971014312196 - res.point.coords[ 0 ].into_inner() ).abs() < 10e-5 );
...
{
  let f = | x : &nelder_mead::Point | ( 1.0 - x.coords[ 0 ].into_inner() ).powi( 2 ) + 100.0 * ( x.coords[ 1 ] - x.coords[ 0 ].powi( 2 )).powi( 2 ) ;
  let mut optimizer: nelder_mead::Optimizer< Range< f64 >, _ > = nelder_mead::Optimizer::new( f );
  ...
  assert!( ( 1.0 - res.point.coords[ 0 ].into_inner() ).abs() < 10e-5 );
  assert!( ( 1.0 - res.point.coords[ 1 ].into_inner() ).abs() < 10e-5 );
...
//     for i in 0..30
//     {
//       y += ( 1.0 - x.coords[ i ].into_inner() ).powi( 2 ) + 100.0 * ( x.coords[ i + 1 ] - x.coords[ i ].powi( 2 )).powi( 2 )
//     }
//     y
//   };
//   let mut optimizer: nelder_mead::Optimizer< Range< f64 >, _ > = nelder_mead::Optimizer::new( f );
...
{
  let f = | x : &nelder_mead::Point | ( x.coords[ 0 ].powi( 2 ) + x.coords[ 1 ].into_inner() - 11.0 ).powi( 2 ) + ( x.coords[ 0 ] + x.coords[ 1 ].powi( 2 ) - 7.0 ).powi( 2 ) ;
  let mut optimizer: nelder_mead::Optimizer< Range< f64 >, _ > = nelder_mead::Optimizer::new( f );
  optimizer.start_point = nelder_mead::Point::new( vec![ 0.0, 0.0 ] );
  ...
  {
    if ( ( minima.0 - res.point.coords[ 0 ].into_inner() ).abs() < 10e-5 ) && ( ( minima.1 - res.point.coords[ 1 ].into_inner() ).abs() < 10e-5 )
    {
