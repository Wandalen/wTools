...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Help requested: {}", cmd.help_invoked);
    ...
    println!("Help requested: {}", cmd2.help_invoked);
    println!("Context arguments: {:?}", cmd2.named_arguments);
    
    assert!(cmd2.help_invoked);
    assert_eq!(cmd2.named_arguments.get("version").unwrap(), "1.2.0");
    
    // Namespace help
    println!("\n=== Namespace Help ===");
    let cmd3 = parser.parse_single_instruction("system ?")?;
    println!("Namespace: {:?}", cmd3.command_path_slices);
    println!("Help requested: {}", cmd3.help_invoked);
    ...
    println!("Help requested: {}", cmd4.help_invoked);
    
    assert!(cmd4.help_invoked);
