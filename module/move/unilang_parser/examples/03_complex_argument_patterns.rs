...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Destination: {}", cmd2.named_arguments.get("destination").map(|arg| &arg.value).unwrap_or(&"not found".to_string()));
    println!("Compress: {}", cmd2.named_arguments.get("compress").map(|arg| &arg.value).unwrap_or(&"not found".to_string()));
