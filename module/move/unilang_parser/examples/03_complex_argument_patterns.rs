...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("All arguments: {:?}", cmd.arguments);
    println!("Named arguments: {:?}", cmd.named_arguments);
    
    // Access different argument types
    if !cmd.arguments.is_empty() {
        println!("First positional argument: {}", cmd.arguments[0]);
    }
    ...
    println!("Source (positional): {}", cmd2.arguments[0]);
    println!("Destination: {}", cmd2.named_arguments.get("destination").unwrap_or(&"not found".to_string()));
    println!("Compress: {}", cmd2.named_arguments.get("compress").unwrap_or(&"not found".to_string()));
