...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for cmd_str in &commands {
        match parser.parse_single_instruction(cmd_str) {
            Ok(instruction) => {
                ...
                let arg_count = instruction.arguments.len() + instruction.named_arguments.len();
                
                if successful_parses <= 3 {  // Only print first few for brevity
    ...
    println!("  Processed {} commands in {:?} ({:.2} μs/command)", 
             successful_parses, duration, duration.as_micros() as f64 / successful_parses as f64);
    ...
    match parser.parse_multiple_instructions(batch_input) {
        Ok(instructions) => {
            ...
            for (i, instruction) in instructions.iter().enumerate() {
                ...
                if i < 2 {  // Only print first couple
                    println!("    Instruction {}: {:?} ({} args)", 
                             i + 1, command_segments, arg_count);
                }
            }
            let process_duration = process_start.elapsed();
            println!("  Processed in {:?} (total: {:?})", 
                     process_duration, parse_duration + process_duration);
        }
    ...
    for cmd in large_command_set.iter().cycle().take(1000) {  // Simulate 1000 commands
        match parser.parse_single_instruction(cmd) {
            Ok(instruction) => {
                processed_count += 1;
                total_args += instruction.arguments.len() + instruction.named_arguments.len();
    ...
    println!("  Processed mixed input: {} success, {} errors in {:?}", 
