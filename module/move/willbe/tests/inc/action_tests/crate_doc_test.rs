...
fn basic_test()
{
  ...
  let crate_dir = CrateDir::try_from( project.as_path() )
  .expect( "Failed to create CrateDir" );
  ...
  assert!( report.status.contains( "successfully" ), "Report status is not successful: {}", report.status );
  ...
  assert!( expected_output_path.is_file(), "Output file not found at expected location: {}", expected_output_path.display() );
  ...
  assert!( content.contains( "# Module `dummy_crate`" ), "Output file missing module header" );
  assert!( content.contains( "## Functions" ), "Output file missing Functions section" );
  assert!( content.contains( "### Function `dummy`" ), "Output file missing function header" );
  assert!( content.contains( "A dummy function." ), "Output file missing function doc comment" );
}
...
fn output_option_test()
{
  ...
  assert!( expected_output_abs_path.is_file(), "Output file not found at expected location: {}", expected_output_abs_path.display() );
  let content = std_fs::read_to_string( &expected_output_abs_path ).expect( "Failed to read output file" );
  assert!( !content.is_empty(), "Output file is empty" );
  assert!( content.contains( "# Crate Documentation" ), "Output file missing main header" );
  assert!( content.contains( &format!( "# Module `{}`", crate_name ) ), "Output file missing module header" );
  assert!( content.contains( "### Function `another`" ), "Output file missing function header" );
  assert!( content.contains( "Another function." ), "Output file missing function doc comment" );

  // Ensure the default file (in target/doc) was NOT created
  assert!( !workspace.target_directory().join("doc").join(format!( "{}_doc.md", crate_name )).exists() );

  // Clean up the created file/directory relative to CWD
  if expected_output_abs_path.exists() { std_fs::remove_file( &expected_output_abs_path ).unwrap(); }
  if expected_output_abs_path.parent().unwrap().read_dir().unwrap().next().is_none()
  {
...
fn non_crate_dir_test()
{
  ...
  assert!( crate_dir_result.is_err(), "CrateDir::try_from should fail for a directory without Cargo.toml" );
}
...
fn cargo_doc_fail_test()
{
  ...
  assert!( matches!( error, CrateDocError::Command( _ ) ), "Expected Command error, got {:?}", error );
  assert!( report.status.contains( &format!( "Failed during `cargo doc` execution for `{}`.", crate_name ) ), "Report status mismatch: {}", report.status );
  assert!( report.cargo_doc_report.is_some() );
  assert!( report.cargo_doc_report.unwrap().error.is_err(), "Cargo doc report should indicate an error" );
