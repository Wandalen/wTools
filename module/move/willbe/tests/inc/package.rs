...
impl TestPackage {
  ...
  pub fn dependency(mut self, name: impl Into<String>) -> Self {
    self.dependencies.push(Dependency::Normal {
      name: name.into(),
      path: None,
      is_macro: false,
    });
  ...
  pub fn _dev_dependency(mut self, name: impl Into<String>) -> Self {
    self.dependencies.push(Dependency::Dev {
      name: name.into(),
      path: None,
      is_macro: false,
    });
...
{}"#,
      ...
      self
        .dependencies
        .iter()
        .map(Dependency::as_toml)
        .fold(String::new(), |acc, d| { format!("{acc}\n\n{d}") })
    );
...
fn kos_plan() {
  ...
  let workspace = TestWorkspace::new(temp.path())
    .unwrap()
    .with_packages([
      TestPackage::new("a"),
  ...
  let plan = action::publish_plan(&the_patterns, channel::Channel::Stable, false, false).unwrap();
  ...
  'sequences: for sequence in expected_one_of {
    for index in 0..5 {
      if *queue[index] != sequence[index].to_string().into() {
        continue 'sequences;
      }
    }
    fail = false;
