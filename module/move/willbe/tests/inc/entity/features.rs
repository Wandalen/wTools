...
#[test]
fn case_1() {
  let package = mock_package(vec![("f1", vec![]), ("f2", vec![]), ("f3", vec![])]);
...
fn estimate() {
  ...
  assert_eq!(
    estimate_with(5, 2, false, false, &["feature1".to_string(), "feature2".to_string()], 2),
    20
  );
  assert_eq!(
    estimate_with(5, 2, true, true, &["feature1".to_string(), "feature2".to_string()], 2),
    22
  );
}
