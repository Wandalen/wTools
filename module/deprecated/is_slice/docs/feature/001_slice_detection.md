...
The mechanism requires no allocator, no collections, and no standard library runtime — only foundational type-system primitives. See `docs/invariant/001_no_std.md`.
...
|------|------|----------------|
| source | `src/lib.rs` | is_slice macro — type-level discrimination mechanism |
| test | `tests/is_slice_tests.rs` | Test root — delegates to slice_tests |
| test | `tests/inc/slice_tests.rs` | 15 cases: variables, literals, arrays, Vec, primitives, Box, empty, nested, struct fields |
