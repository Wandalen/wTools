//! Smoke testing of the `fs_tools` alias crate.
//!
//! Verifies that `file_tools` public API is correctly re-exported through `fs_tools`.
...
//!
//! - ✅ Module re-export accessible (`fs_tools::path`, `fs_tools::fs`)
...
///
/// Calls `fs_tools::path::file_upward_find` starting from the current
...
fn local_smoke_test()
{
  let result = fs_tools::path::file_upward_find
  (
    std::path::Path::new( "." ),
    "Cargo.toml",
    10,
...
///
/// Constructs a `fs_tools::fs::TempDir`, sets its public path fields,
...
fn published_smoke_test()
{
  ...
  tmp.prefix_path = std::path::PathBuf::from( "fs_tools_smoke_" );
  assert!(
