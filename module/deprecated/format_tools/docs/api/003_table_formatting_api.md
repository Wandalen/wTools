...
- **Purpose**: Provide interfaces and utilities for formatting structured data as a formatted plain-text table.
- **Responsibility**: Documents the public interface for table formatting — the format interface, the table conversion interface, raw construction, and compatibility policy.
- **In Scope**: The pluggable output format interface, the table conversion interface, and the raw vector table construction function.
...
**Table conversion interface**: An interface implemented by types that can present themselves as tables. Provides methods to format the type as a table string using any compatible output format. Works with types implementing the reflection interface from reflect_tools.
...
The pluggable format interface is stable; new implementations can be added without breaking existing callers. The table conversion interface is stable. Raw construction function signature is stable.
