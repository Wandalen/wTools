...
//!
//! Verifies that the `glob` crate is properly re-exported through `file_tools`
//! and all functionality is accessible.
//!
//! ## Design Decisions
//!
//! **Why module-only re-export (`pub use ::glob;`)?**
//! Cannot have both `file_tools::glob` as module AND `file_tools::glob` as function.
//! Re-exporting the module avoids name collision. Users access via `file_tools::glob::glob()`.
//!
...
{
  // Verify glob module is accessible via file_tools
  use the_module::glob;
