...
**⚠️ SUPERSEDED**: After analysis, custom wrapper extraction was deemed unnecessary. The glob crate provides comprehensive functionality and file_tools now re-exports it directly via `file_tools::glob`. See [Task 002](./002_reexport_glob_crate.md).
...
Extract glob pattern detection and expansion utilities from the wplan ecosystem to `file_tools`, making them available to all wTools projects. These utilities detect glob patterns in strings and expand them to directory/file paths - fundamental for any CLI tool that accepts file patterns.
...
### Why Extract to file_tools

1. **Universal Need**: Every CLI tool needs glob pattern handling for file arguments
2. **Foundation**: file_tools is the natural home for filesystem pattern matching
...
```rust
//! Glob pattern utilities for file_tools
//!
//! Provides:
...
///
/// ```rust
/// use file_tools::glob::is_glob_pattern;
///
...
///
/// ```rust
/// use file_tools::glob::is_glob_pattern_strict;
///
...
///
/// ```rust
/// use file_tools::glob::validate_pattern;
///
...
///
/// ```rust
/// use file_tools::glob::{ expand_with, GlobOptions };
///
...
///
/// ```rust
/// use file_tools::glob::expand_dirs;
///
...
///
/// ```rust
/// use file_tools::glob::expand_files;
///
...
///
/// ```rust
/// use file_tools::glob::expand_many;
///
...
///
/// ```rust
/// use file_tools::glob::has_matches;
///
/// if has_matches( "*.rs" )?
/// {
...
///
/// ```rust
/// use file_tools::glob::count_matches;
///
...
**Tasks**:
1. Create `file_tools/src/glob/pattern.rs`
...
**Tasks**:
1. Create `file_tools/src/glob/expand.rs`
...
**Tasks**:
1. Update `file_tools/src/lib.rs` to export glob module
2. Update `file_tools/Cargo.toml` dependencies
3. Migrate wplan_client to use new API
4. Delete old implementations
5. Verify all tests pass

**Acceptance Criteria**:
- [ ] All modules exported from `file_tools::glob`
- [ ] wplan_client uses `file_tools::glob::*`
- [ ] Old code deleted
- [ ] All tests pass
...
- [ ] `cargo test -p file_tools` passes
