...
Re-export the `glob` crate from `file_tools` to provide Unix shell-style pattern matching functionality. This is a clean re-export approach rather than extracting custom wrapper functions (which was the original task 001 proposal).
...
| ID   | Test                           | Coverage                              |
|------|--------------------------------|---------------------------------------|
| G1.1 | glob_module_accessible         | Module accessibility via file_tools     |
...
```rust
use file_tools::dependency::glob;
...
Original plan included convenience re-exports like `pub use ::glob::glob;` but this would create a name collision - `file_tools::glob` would be ambiguous (module vs function). Module-only re-export (`pub use ::glob;`) avoids this.
...
4. **wplan can use glob directly** - No need for file_tools intermediary for custom wrappers
...
2. **Name Collision Avoidance**: Cannot have `file_tools::glob` as both module AND function. Module-only re-export (`pub use ::glob;`) avoids this.
