...
//! only internal `tree_fmt` rendering (which handles padding internally).
//!
//! # Fix Applied
//!
...
use tree_fmt::{ truncate_cell, pad_to_width };
...
fn bug_reproducer_issue_003_cjk_alignment()
{
  ...
  // Pad both to 10 characters using tree_fmt's function
  let padded_ascii = pad_to_width( ascii_text, 10, false );
  let padded_cjk = pad_to_width( cjk_text, 10, false );

  println!( "Character-based padding (current tree_fmt behavior):" );
  ...
  // - tree_fmt pads based on char count (3 chars → 10 chars)
