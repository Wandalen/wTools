...
Provide a single `Format` trait and canonical `TableView` data type so callers can write format-agnostic code while each formatter lives behind its own feature flag for zero-cost abstractions.
