...
- **Version:** 0.1
- **Date:** 2025-12-04
- **Project Name:** claude_runner - Claude Code Process Execution
- **Type:** Rust Library Crate

## Workspace Affiliation

- **Workspace:** wtools (`~/pro/lib/wip_core/wtools/dev`)
- **Location within workspace:** `module/core/claude_runner`
...
claude_runner owns **Claude Code process execution** exclusively. It does NOT manage session storage paths.

**Separation of Concerns:**
- **claude_runner** (THIS crate): Process execution, Command::new("claude"), builder pattern
- **claude_session**: Session storage paths, continuation detection

### Builder Pattern Over Factories

claude_runner uses fluent builder API instead of deprecated factory methods.
...
claude_runner depends ONLY on error_tools for error handling. No other external dependencies.
...
claude_runner must provide ClaudeCommand::new() as the sole construction method. Factory methods (generate) are deprecated.

### FR-2: Fluent Builder API
claude_runner must provide with_*() methods for all Claude Code configuration parameters (40+ parameters).

### FR-3: Token Limit Configuration
claude_runner must provide with_max_output_tokens() method with 200,000 token default to fix token limit bug.

### FR-4: Working Directory Configuration
claude_runner must provide with_working_directory() method to set process working directory.

### FR-5: Continuation Flag Support
claude_runner must provide with_continue_conversation() method to add `-c` flag when true.

### FR-6: Single Execution Point
claude_runner must consolidate all Command::new("claude") calls into single execute() method. Duplication factor must equal 1.

### FR-7: Process Output Capture
claude_runner must capture and return stdout/stderr/exit_code from Claude Code process execution via `ExecutionOutput` struct.

### FR-8: Error Handling
claude_runner must use error_tools for error handling and provide clear error messages for common failures (Claude not in PATH, permission denied, exit codes).

### FR-9: Interactive Mode Support
claude_runner must provide execute_interactive() method that allows Claude Code to take over the terminal (TTY attached) without capturing output. Non-interactive execute() method captures output.

### FR-10: Bash Timeout Configuration
claude_runner must provide with_bash_timeout_ms() and with_bash_max_timeout_ms() methods with defaults of 3,600,000ms (1 hour) and 7,200,000ms (2 hours) respectively to prevent premature timeout failures in automation.

### FR-11: Auto-Continue Configuration
claude_runner must provide with_auto_continue() method with default of true to enable programmatic automation without manual prompts.

### FR-12: Telemetry Configuration
claude_runner must provide with_telemetry() method with default of false to disable telemetry collection in automation contexts.

### FR-13: Auto-Approve Tools Configuration
claude_runner must provide with_auto_approve_tools() method with default of false (opt-in only for security).

### FR-14: Action Mode Configuration
claude_runner must provide with_action_mode() method accepting ActionMode enum (Ask/Allow/Deny) with default of Ask.

### FR-15: Log Level Configuration
claude_runner must provide with_log_level() method accepting LogLevel enum with default of Info.

### FR-16: Temperature Configuration
claude_runner must provide with_temperature() method with default of 1.0 for model temperature control.

### FR-17: Sandbox Mode Configuration
claude_runner must provide with_sandbox_mode() method with default of true for security.

### FR-18: Session Directory Configuration
claude_runner must provide with_session_dir() method for explicit session storage override.

### FR-19: Top-P/Top-K Configuration
claude_runner must provide with_top_p() and with_top_k() methods for model sampling control.

### FR-20: Skip Permissions Configuration
claude_runner must provide with_skip_permissions() typed builder method to add `--dangerously-skip-permissions` flag. Default is false.

### FR-21: Command Inspection
claude_runner must provide describe() method returning human-readable command line string for dry-run and debug output.
...
claude_runner must provide describe_env() method returning NAME=VALUE lines for all configured environment variables.
...
claude_runner execute() must return ExecutionOutput struct with stdout, stderr, and exit_code fields instead of raw String.

## Non-Functional Requirements

### NFR-1: Minimal Dependencies
claude_runner must depend only on: error_tools (error handling), standard library. Total: 1 workspace dependency (wtools workspace), 0 external direct dependencies.
...
claude_runner must provide six-layer verification pyramid proving migration completeness through objective, measurable evidence.
...
**CRITICAL:** claude_runner owns execution ONLY. Storage paths come from claude_session.

```text
dream_agent → claude_runner (execution) + claude_session (storage paths)
              ↓                             ↓
              execute() method              resolve_storage_path()
              ↓
              Command::new("claude")        (ONLY in claude_runner)
```

**Responsibility Rules:**
1. claude_runner owns Command::new("claude") ✅
2. claude_runner owns process execution ✅
3. claude_session provides storage paths ✅
4. dream_agent configures via builder ✅
5. NO Command::new("claude") outside claude_runner ✅
...
erDiagram
    ...
    CLAUDE_RUNNER {
        string crate_name "claude_runner"
...
3. **Execution separation** - claude_runner executes, claude_session provides paths (via dream_agent)
...
- That occurrence is in claude_runner/src/command.rs
...
**After Migration:**
- Command::new("claude") appears 1x in codebase (claude_runner::execute())
- Duplication factor: 1x
- Token limit: Explicit 200K (fixes bug)
- API: ClaudeCommand::new().with_*().execute() builder pattern
- Clean separation: claude_runner executes, claude_session provides paths

**Files:**
- claude_runner/Cargo.toml
- claude_runner/src/lib.rs
- claude_runner/src/command.rs (ClaudeCommand builder + execute() + Command::new("claude"))
- claude_runner/src/types.rs (ActionMode, LogLevel, ExecutionOutput)
- claude_runner/tests/ (20 test files — builder, verification, describe, skip_permissions, etc.)

**Verification:**
- grep -r "Command::new.*claude" entire workspace = exactly 1 match in claude_runner
