...
fn session_filter_min_entries()
{
  ...
  for session in filtered
  {
    ...
    assert!( count >= 10, "Session has {} entries, expected >= 10", count );
  }
}
...
fn session_filter_id_substring()
{
  ...
  for session in &filtered
  {
    assert!( matcher.matches( session.id() ), "Session ID {} should contain {}", session.id(), substring );
  }
}
...
fn project_filter_path_substring()
{
  ...
  for project in &filtered
  {
    ...
    assert!( matcher.matches( &path_str ), "Project path {} should contain 'willbe'", path_str );
  }
}
...
fn project_filter_min_sessions()
{
  ...
  for project in filtered
  {
    ...
    assert!( count >= 5, "Project has {} sessions, expected >= 5", count );
  }
}
