...
#[test]
#[ignore] // Ignored by default since it requires specific file to exist
fn test_parse_real_problematic_file()
{
  ...
  for (i, line) in lines.iter().enumerate()
  {
    ...
    if let Err( e ) = result
    {
      println!( "Line {}: FAILED with error: {:?}", i, e );
      ...
      if let Some( pos_str ) = e.to_string().split( "position " ).nth( 1 )
      {
        if let Some( pos ) = pos_str.split( ':' ).next().and_then( | s | s.parse::< usize >().ok() )
        {
          if pos < line.len()
          {
            ...
            println!( "Context around position {}: ...{}...", pos, context );

            // Show actual bytes
            let bytes = &line.as_bytes()[ start..end ];
            println!( "Bytes (hex): {:?}", bytes.iter().map( | b | format!( "{:02x}", b ) ).collect::< Vec< _ > >() );
          }
        }
      }
