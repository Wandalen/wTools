...
impl Session
{
  ...
  fn load_entries( &mut self ) -> Result< ()>
  {
    let content = fs::read_to_string( &self.storage_path )
      ...
      (
        e,
        format!( "reading session file: {:?}", self.storage_path )
      ))?;
    ...
    for line in content.lines()
    {
      ...
      // Try to parse as conversation entry, skip if it's a metadata entry
      match Entry::from_json_line( line )
      {
        Ok( entry ) => entries.push( entry ),
        Err( _ ) =>
        {
          // Silently skip non-conversation entries (queue-operation, summary, etc.)
          // This is graceful degradation - we only care about conversation entries
          continue;
        }
      }
    }
  ...
  pub fn append_entry( &mut self, entry : &Entry ) -> Result< ()>
  {
    ...
    let mut file = fs::OpenOptions::new()
      ...
      (
        e,
        format!( "opening session file for append: {:?}", self.storage_path )
      ))?;
    ...
    writeln!( file, "{}", json_line )
      ...
      (
        e,
        format!( "appending to session file: {:?}", self.storage_path )
      ))?;
