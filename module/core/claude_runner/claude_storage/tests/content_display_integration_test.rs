...
fn show_displays_content_by_default()
{
  ...
  println!( "Testing with session: {} in project: {}", session_id, project_path );
  ...
  {
    line.trim().chars().next().map( | c | c.is_ascii_digit() ).unwrap_or( false )
      && line.contains( "[User]" )
...
fn show_metadata_only_parameter()
{
  ...
  let session_id = session_id.unwrap();
  let project_path = project_path.unwrap();
  println!( "Testing metadata::1 with session: {} in project: {}", session_id, project_path );
