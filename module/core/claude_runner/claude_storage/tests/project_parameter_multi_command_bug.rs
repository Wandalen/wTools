...
{"type":"assistant","uuid":"uuid-2","parentUuid":"uuid-1","timestamp":"2025-12-06T10:00:01Z","cwd":"/tmp","sessionId":"test","version":"2.0.0","gitBranch":"main","userType":"external","isSidechain":false,"message":{"role":"assistant","content":[{"type":"text","text":"response"}]}}"# ).unwrap();
  ...
  assert!(
    ...
    "Bug: .count treats path project as UUID. stderr: {}",
    stderr
  );

  assert!(
    output.status.success(),
    ".count should succeed with path project. stderr: {}",
    stderr
  );

  assert!(
    stdout.contains( "1" ),
    "Should count 1 session. stdout: {}",
    stdout
  );
}
...
{"type":"assistant","uuid":"uuid-2","parentUuid":"uuid-1","timestamp":"2025-12-06T10:00:01Z","cwd":"/tmp","sessionId":"test","version":"2.0.0","gitBranch":"main","userType":"external","isSidechain":false,"message":{"role":"assistant","content":[{"type":"text","text":"entry 2"}]}}"# ).unwrap();
  ...
  assert!(
    !stderr.contains( "Project not found" ),
    "Bug: .count entries treats path project as UUID. stderr: {}",
    stderr
  );

  assert!(
    output.status.success(),
    ".count entries should succeed with path project. stderr: {}",
    stderr
  );

  assert!(
    stdout.contains( "2" ),
    "Should count 2 entries. stdout: {}",
    stdout
  );
}
...
fn test_search_with_path_project()
{
  ...
  assert!(
    !stderr.contains( "Project not found" ),
    "Bug: .search treats path project as UUID. stderr: {}",
    stderr
  );

  assert!(
    output.status.success(),
    ".search should succeed with path project. stderr: {}",
    stderr
  );
}
...
fn test_export_with_path_project()
{
  ...
  assert!(
    !stderr.contains( "Project not found" ),
    "Bug: .export treats path project as UUID. stderr: {}",
    stderr
  );

  assert!(
    output.status.success(),
    ".export should succeed with path project. stderr: {}",
    stderr
  );
}
