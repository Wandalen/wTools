...
fn test_count_default_behavior_context_aware()
{
  ...
  assert!(
    ...
    ".count command should succeed. stderr: {}, stdout: {}",
    stderr, stdout
  );
  ...
  assert_eq!(
    count_str, "5",
    "Expected .count (no params) to return 5 entries in current project, got: {}",
    count_str
  );
}
...
fn test_count_with_path_project_parameter()
{
  ...
  assert_eq!(
    count_str, "3",
    "Expected .count with path parameter to return 3 sessions, got: {}",
    count_str
  );
}
