...
fn test_export_session_id_required()
{
  ...
  assert!(
    ...
    "Should fail when session_id missing. Got: {}",
    combined
  );
  ...
  assert!(
    ...
    "Error should mention session_id is required. Got: {}",
    combined
  );
}
...
fn test_export_output_required()
{
  ...
  assert!(
    !output.status.success(),
    "Should fail when output missing. Got: {}",
    combined
  );
  ...
  assert!(
    ...
    combined.to_lowercase().contains( "required" ),
    "Error should mention output is required. Got: {}",
    combined
  );
}
...
fn test_export_format_invalid()
{
  ...
  assert!(
    !output.status.success(),
    "Should fail with invalid format. Got: {}",
    combined
  );
  ...
  assert!(
    ...
    "Error should mention format validation. Got: {}",
    combined
  );
}
...
fn test_export_format_valid()
{
  for format in [ "markdown", "json", "text" ]
  {
    ...
    assert!(
      !( combined.to_lowercase().contains( "format" ) &&
         combined.to_lowercase().contains( "invalid" ) ),
      "Should not fail on format validation for '{}'. Got: {}",
      format,
      combined
    );
  }
...
fn test_export_output_directory_nonexistent()
{
  ...
  assert!(
    !output.status.success(),
    "Should fail with nonexistent directory. Got: {}",
    combined
  );
  ...
  assert!(
    ...
    combined.to_lowercase().contains( "not found" ) ||
    combined.to_lowercase().contains( "does not exist" ),
    "Error should mention directory/path issue. Got: {}",
    combined
  );
}
...
fn test_export_with_required_parameters()
{
  ...
  assert!(
    !( combined.to_lowercase().contains( "required" ) &&
       ( combined.to_lowercase().contains( "session" ) ||
         combined.to_lowercase().contains( "output" ) ) ),
    "Should not fail on parameter validation with both required params. Got: {}",
    combined
  );
}
...
fn test_export_default_format_markdown()
{
  ...
  assert!(
    !( combined.to_lowercase().contains( "format" ) &&
       combined.to_lowercase().contains( "invalid" ) ),
    "Default format should be valid. Got: {}",
    combined
  );
}
