...
fn test_command_version_consistency()
{
  ...
  for ( name, version ) in &commands
  {
    assert!(
      ...
      "Command '{}' version ({}) exceeds crate version ({})",
      name, version, crate_version
    );
  }
  ...
  for ( name, version ) in &commands
  {
    ...
    assert!(
      parts.len() >= 2,
      "Command '{}' version '{}' is not a valid semantic version (must have at least major.minor)",
      name, version
    );
    ...
    for part in parts
    {
      assert!(
        ...
        "Command '{}' version '{}' contains non-numeric component '{}'",
        name, version, part
      );
    }
  }
