...
- **Purpose**: Provide a macro and interface for converting values to strings using a prioritized formatter chain.
...
- **In Scope**: The conversion macro, the fallback interface, and the return type semantics.
...
**Fallback interface**: Implemented automatically for any value paired with a compatible strategy chain. The interface method converts the value using the configured strategy and returns a string container. Callers typically invoke the macro rather than calling the interface method directly.
...
The macro interface is stable. Strategy markers (see api/004) are stable. The return type — a borrowed-or-owned string container — is stable. The interface is public but callers are not expected to implement it manually; the macro handles dispatch automatically.

### Cross-References

| Type | File | Responsibility |
|------|------|----------------|
| source | `src/format/to_string_with_fallback.rs` | Macro and interface implementation |
