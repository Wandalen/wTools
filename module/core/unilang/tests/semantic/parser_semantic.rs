...
fn create_file_processor_command() -> CommandDefinition
{
  CommandDefinition::former()
    ...
    .routine_link( Some( "file_processor".to_string() ) )
    .namespace( "" )
        ...
        for ( i, file_value ) in file_list.iter().enumerate()
        {
          match file_value {
            ...
            Value::List( inner_list ) => {
              // Handle nested lists from Kind::List
              if let Some( Value::String( file_path ) ) = inner_list.first() {
                writeln!( &mut output, "  {}: {file_path}", i + 1 ).unwrap();
              }
            },
            _ => {}
    ...
    Value::List( files ) =>
    {
      ...
      for ( i, expected ) in expected_files.iter().enumerate()
      {
        match &files[i]
        {
          ...
          Value::List( inner_list ) => {
            // With Kind::List, each file value is wrapped in a list
            assert_eq!( inner_list.len(), 1, "Each file should have exactly one value" );
            match &inner_list[0] {
              Value::String( actual ) => assert_eq!( actual, expected, "File {} should match", i + 1 ),
              _ => panic!( "File {} inner value should be a String", i + 1 ),
            }
          },
          _ => panic!( "File {} should be a String or List value", i + 1 ),
        }
      }
    },
...
fn test_backward_compatibility_single_parameter_integration()
{
  let single_param_cmd = CommandDefinition::former()
    ...
    .routine_link( Some( "single_file_processor".to_string() ) )
    .namespace( "" )
...
#[test]
#[ allow( clippy::too_many_lines ) ] // Test functions are naturally verbose with setup/execution/verification
fn test_quote_handling_integration()
{
  let quote_test_cmd = CommandDefinition::former()
    ...
    .routine_link( Some( "quote_processor".to_string() ) )
    .namespace( "" )
      ...
      if let Value::List( cmd_list ) = commands
      {
        ...
        for ( i, cmd_value ) in cmd_list.iter().enumerate()
        {
          match cmd_value {
            ...
            Value::List( inner_list ) => {
              // Handle nested lists from Kind::List
              if let Some( Value::String( cmd_str ) ) = inner_list.first() {
                writeln!( &mut output, "  Command {}: '{cmd_str}'", i + 1 ).unwrap();
              }
            },
            _ => {}
    ...
    Value::List( commands ) =>
    {
      ...
      for ( i, expected_cmd ) in expected.iter().enumerate()
      {
        match &commands[i]
        {
          ...
          Value::List( inner_list ) => {
            // With Kind::List, each command value is wrapped in a list
            assert_eq!( inner_list.len(), 1, "Each command should have exactly one value" );
            match &inner_list[0] {
              Value::String( actual ) => assert_eq!( actual, expected_cmd, "Command {} should preserve quotes and spaces", i + 1 ),
              _ => panic!( "Command {} inner value should be a String", i + 1 ),
            }
          },
          _ => panic!( "Command {} should be a String or List", i + 1 ),
        }
      }
    },
...
fn test_multiple_parameter_performance()
{
  ...
  let perf_test_cmd = CommandDefinition::former()
    ...
    .routine_link( Some( "perf_processor".to_string() ) )
    .namespace( "" )
