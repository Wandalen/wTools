...
fn setup_test_registries() -> Result< ( StaticCommandRegistry, CommandRegistry ), Box< dyn std::error::Error > >
{
  ...
  // Note: In real usage, this would be generated by build.rs from YAML
  // Using the static map defined at module level
  let static_registry = StaticCommandRegistry::from_commands( &MOCK_STATIC_MAP );
