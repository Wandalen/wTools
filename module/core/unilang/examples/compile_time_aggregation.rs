...
macro_rules! compile_time_cli {
  ...
  // Generate static command definitions at compile-time
  // The internal implementation uses optimized static maps for O(1) lookup.
  // This would be populated by the macro expansion processing all module
  // commands with prefixes applied.
  //
  // Users interact with the public API only - implementation details hidden:
  static AGGREGATED_COMMANDS: StaticCommandMap = StaticCommandMap::empty();
...
pub mod generated_example {
  ...
  // Users interact with the public API only - implementation details hidden.
  //
  // Example of the public interface (generated by build.rs):
  //
  // ```rust
  // use unilang::static_data::StaticCommandMap;
  //
  ...
  // let registry = StaticCommandRegistry::from_commands(&UNIFIED_CLI_COMMANDS);
  // ```
