//! Core data structures for `data_fmt`
...
{
  /// Column values for this node (each cell may carry an optional ANSI color).
  pub columns : Vec< DecoratedText >,
}
...
impl ColumnData
{
  ...
  pub fn from_pairs( pairs : Vec< ( &str, &str ) > ) -> Self
  {
    Self
    {
      columns : pairs.into_iter().map( | ( _, v ) | DecoratedText::from( v ) ).collect()
    }
  }
...
///
/// ```
/// use data_fmt::{ TableView, TableMetadata };
///
/// let view = TableView {
...
pub struct TableView
{
  ...
  /// Table rows (each cell may carry an optional ANSI color via `DecoratedText`).
  pub rows : Vec< Vec< DecoratedText > >,
...
impl TableView
{
  ...
  pub fn new( metadata : TableMetadata, rows : Vec< Vec< DecoratedText > > ) -> Self
  {
