...
///
/// ```
/// use data_fmt::{ TreeBuilder, conversions };
///
...
///
/// ```
/// use data_fmt::{ TreeBuilder, conversions::{ flatten_to_table_tree_with_config, FlattenConfig } };
///
...
)
{
  ...
  if config.include_data { row.push( data_str.into() ); }
