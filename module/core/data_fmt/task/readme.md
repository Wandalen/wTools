...
| 14 | 014 | 1440 | 8 | 5 | 9 | 4 | ✅ (Completed) | any | [Implement remaining 168 spec tests](completed/014_implement_remaining_spec_tests.md) | Implement 168 test functions from 43 spec files (data_structure/1, input_model/2, input_type/2, pattern/4, trait/1, variant/33) |
...
| 16 | 016 | 1344 | 7 | 6 | 8 | 4 | ✅ (Completed) | any | [Add feature-gated `quantity` formatting module (duration/age, number, byte size)](completed/016_add_quantity_formatting_module.md) | Fixed 6-visible-char `NNuNNu` duration/age formatter + number/byte formatters, gray unit letters, NO_COLOR/TTY-aware, feature-gated (`quantity`), minor bump 0.8.1→0.9.0; consumer wplan TSK-1509 |
| 17 | 017 | 1152 | 6 | 8 | 8 | 3 | ✅ (Completed) | any | [Extend `quantity` with SI (decimal, 1000-based) byte formatting](completed/017_extend_quantity_si_bytes.md) | SI/1000-based byte formatter (`bytes_si`), base-1000 sibling of `bytes_human`; revives 016's deferred decimal bytes; consumer glassbox via wplan TSK-1539; minor bump 0.11.0→0.12.0 |
| 18 | 018 | 1764 | 7 | 7 | 9 | 4 | ✅ (Completed) | ai | [Header and footer titled-rule support across formatters](completed/018_heading_footer_multi_formatter.md) | Generalized `Heading` from header-only/Table-only to header+footer across Table/Tree/Expanded/Text/Yaml/Toml/Sql via one shared formatter-agnostic render primitive; `Json`/`Logfmt`/`Html` excluded per scope; 998/998 tests pass, clippy clean, no version bump |
| 19 | 019 | 1296 | 6 | 8 | 9 | 3 | ✅ (Completed) | ai | [HtmlFormatter heading and footer support](completed/019_html_heading_footer.md) | Closes task 018's deferred Html item — `render_commented_rule_if_present` gains a `comment_suffix` parameter so `<!-- -->`'s closing delimiter is always emitted; `HtmlFormatter::with_heading()`/`with_footer()`; cfg-gate verified clean across 6 feature combinations; 1004/1004 tests pass, clippy clean, no version bump |
| 20 | 020 | 2016 | 7 | 9 | 8 | 4 | ✅ (Completed) | ai | [Extend `bytes_human` with a `TB` tier](completed/020_extend_bytes_human_tb_tier.md) | Restores the sibling symmetry 017 designed for — `bytes_si` has `TB`, `bytes_human` stops at `GB`, so 2 TiB renders `2048.00 GB`; no other `data_fmt` function offers base-1024 + verbose + `TB`; unblocks yrd_core BUG-1584 (`wrun_core`) and TSK-1540 AC-1 (`will_clean`); minor bump 0.13.3→0.14.0 |
