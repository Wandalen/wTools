...
- **When:** The caption line is rendered.
- **Then:** The total character count of the caption line (measured by Unicode scalar values, not bytes) equals exactly `table_width`; the trailing rule characters account for the difference between `table_width` and the sum of lead prefix width, content character count, and the single space that follows the content.
- **Note:** Covered by FC-3 (`caption_fills_to_table_width_fc3` — pending table_width implementation via TSK-008).
...
- **Note:** (a) covered by `caption_content_equals_table_width_no_trailing_rule_ft4` (pending table_width implementation via TSK-008); (b) covered by `caption_trail_clamped_to_zero_when_content_too_wide_fc4`; (c) covered by `caption_title_exceeds_table_width_no_trailing_rule_ft7` (also verifies content verbatim — no truncation).
...
- **Then:** The measured count equals `table_width`; measuring by byte length would produce a different (larger) result because `─` (U+2500) is 3 bytes and `·` (U+00B7) is 2 bytes in UTF-8.
- **Note:** Covered by FC-3 (`caption_fills_to_table_width_fc3` — pending table_width implementation via TSK-008; verify fill using `.chars().count()`).

---

### AC-7: empty content string: no separator emitted; trailing rule fills remaining width

- **Given:** A `Heading::new("")` with no additional fields applied to a table whose rendered display width (`table_width`) equals 20.
- **When:** The caption line is rendered.
- **Then:** The content string is empty (zero visible characters); no middle-dot separator (`·`) appears anywhere in the output; the trailing rule fills the remaining width from column 4 to column 20 (15 rule characters, since lead = 4 + space = 1 → trail = 20 − 5 = 15); the total character count equals exactly 20; no panic occurs from empty-string arithmetic.
