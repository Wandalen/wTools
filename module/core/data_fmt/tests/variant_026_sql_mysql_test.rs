//! Variant 026: SQL `MySQL` spec tests (VT-1..VT-4)

#![ cfg( all( feature = "enabled", feature = "sql_mysql" ) ) ]

use data_fmt::{ RowBuilder, SqlFormatter, SqlVariant, Format };

/// VT-1: output uses `MySQL` backtick quoting
// test_kind: spec_case(VT-1)
#[ test ]
fn variant_026_vt_01_mysql_backtick_quoting()
{
  let view = RowBuilder::new( vec![ "Name".into(), "Age".into() ] )
    .add_row( vec![ "Alice".into(), "30".into() ] )
    .build_view();

  let out = Format::format(
    &SqlFormatter::with_variant( "users", SqlVariant::MySQL ),
    &view,
  ).unwrap();

  assert!( out.contains( '`' ), "backtick quoting present: {out}" );
}

/// VT-2: column identifiers are backtick-quoted
// test_kind: spec_case(VT-2)
#[ test ]
fn variant_026_vt_02_backtick_identifiers()
{
  let view = RowBuilder::new( vec![ "user name".into(), "age".into() ] )
    .add_row( vec![ "Alice".into(), "30".into() ] )
    .build_view();

  let out = Format::format(
    &SqlFormatter::with_variant( "users", SqlVariant::MySQL ),
    &view,
  ).unwrap();

  assert!(
    out.contains( "`user name`" ) || out.contains( "`age`" ),
    "backtick-quoted identifiers: {out}",
  );
}

/// VT-3: valid `MySQL` INSERT syntax
// test_kind: spec_case(VT-3)
#[ test ]
fn variant_026_vt_03_valid_insert_syntax()
{
  let view = RowBuilder::new( vec![ "A".into(), "B".into() ] )
    .add_row( vec![ "1".into(), "2".into() ] )
    .build_view();

  let out = Format::format(
    &SqlFormatter::with_variant( "data", SqlVariant::MySQL ),
    &view,
  ).unwrap();

  assert!( out.contains( "INSERT INTO" ), "INSERT INTO present: {out}" );
  assert!( out.contains( "VALUES" ), "VALUES present: {out}" );
}

/// VT-4: empty table produces no value tuples
// test_kind: spec_case(VT-4)
#[ test ]
fn variant_026_vt_04_empty_table()
{
  let view = RowBuilder::new( vec![ "Col".into() ] )
    .build_view();

  let out = Format::format(
    &SqlFormatter::with_variant( "t", SqlVariant::MySQL ),
    &view,
  ).unwrap();

  let after_values = out.split( "VALUES" ).nth( 1 ).unwrap_or( "" );
  assert!( !after_values.contains( '(' ), "no value tuples for empty table: {out}" );
}
