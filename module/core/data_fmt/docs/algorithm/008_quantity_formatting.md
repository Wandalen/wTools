...
- **In Scope**: `duration_6ch` band selection and clamp, `number_compact` SI scaling, `bytes_iec` IEC scaling, mantissa rendering, `QuantityStyle` and the `NO_COLOR`/TTY resolution policy.
...
| [`src/quantity/number.rs`](../../src/quantity/number.rs) | `number_compact()`, `bytes_iec()`, `format_mantissa()` — magnitude scaling |
