...
3. Create `docs/input_model/NNN_name.md` with the following sections in order per **Type-Specific Requirements** in `readme.md`: `### Scope`, typed cross-reference H3 sections (see existing instances), `### Data Shape`, `### Downstream Connections`.
...
6. Update the **Instances** count in `docs/entity.md` **Master Doc Entities Table** for the `input_model/` row.
