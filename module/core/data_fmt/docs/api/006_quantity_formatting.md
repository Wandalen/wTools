...
All ten formatter signatures are stable — `(value : u64, style : QuantityStyle) -> String` (or `&str` for `parse_duration`'s input). `duration_6ch`'s 6-visible-column output width is a stable invariant callers may depend on for alignment. Digits are never ANSI-styled under `Colored`; only unit letters are — a `Colored` result and its `Plain` counterpart always have identical visible glyph counts. `bytes_human` topping out at `GB` and `bytes_si` extending to `TB` are both stable, intentional divergences — not a gap to be reconciled.
