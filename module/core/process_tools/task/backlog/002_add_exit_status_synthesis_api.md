...
**API Surface**
- [ ] C1 — Does `process_tools/src/exit_status.rs` exist with `mod private {}` block?
- [ ] C2 — Does `synthetic_exit_status(code: i32) -> ExitStatus` exist and use cfg-gated internals?
- [ ] C3 — Do `synthetic_success_status()` and `synthetic_failure_status()` convenience functions exist?
- [ ] C4 — Is the module registered as `layer exit_status;` in `lib.rs` mod_interface block?

**Tests**
- [ ] C5 — Does `process_tools/tests/exit_status_test.rs` exist?
- [ ] C6 — Does it cover all 5 test matrix scenarios (T01–T05)?
- [ ] C7 — Does `cargo test -p process_tools --all-features` pass with zero failures?

**Code Quality**
- [ ] C8 — Does the module use 2-space indents (custom codestyle)?
- [ ] C9 — Are there zero `#[cfg(test)]` blocks in `src/exit_status.rs`?
- [ ] C10 — Is platform-specific code confined to `#[cfg]` blocks inside function bodies only?
