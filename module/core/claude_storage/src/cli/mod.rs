  ...
  for mut project in projects
  {
    ...
    match verbosity
    {
      ...
      1 =>
      {
        ...
        writeln!( output, "{:?} ({} sessions)", project.id(), session_count ).unwrap();
      }
      _ =>
      {
...
) -> core::result::Result< OutputData, ErrorData >
{
  ...
  writeln!( output, "Session: {} ({} entries)", session_id, stats.total_entries ).unwrap();
