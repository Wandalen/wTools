...
{"type":"assistant","text":"test response"}"# ).unwrap();
  ...
  let test_path_display = test_path.display().to_string();
  // Execute .show.project
  let output = common::claude_storage_cmd()
    .args( [ ".show.project",
      &format!( "project::{test_path_display}" )
    ] )
    .output()
    .unwrap();
  ...
  assert!(
    ...
    "Should show session count. stdout: {stdout}"
  );
...
fn test_show_project_from_list_output()
{
  ...
  assert!(
    output.status.success(),
    "Should handle Debug format from .list. stderr: {stderr}"
  );

  assert!(
    stdout.contains( &test_path.to_string_lossy().to_string() ) || stdout.contains( "Project:" ),
    "Should parse Path(...) format. stdout: {stdout}"
  );
}
