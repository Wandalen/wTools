...
fn test_session_filter_auto_enables_display()
{
  ...
  let file1 = project_dir.join( format!( "{matching_session}.jsonl" ) );
  fs::write( &file1, r#"{"type":"user","text":"test"}"# ).unwrap();

  let file2 = project_dir.join( format!( "{non_matching_session}.jsonl" ) );
...
fn test_agent_filter_auto_enables_display()
{
  ...
  let file1 = project_dir.join( format!( "{agent_session}.jsonl" ) );
  fs::write( &file1, r#"{"type":"user","text":"test"}"# ).unwrap();

  let file2 = project_dir.join( format!( "{main_session}.jsonl" ) );
  fs::write( &file2, r#"{"type":"user","text":"test"}"# ).unwrap();
...
fn test_min_entries_filter_auto_enables_display()
{
  ...
  assert!(
    ...
    "Sessions should be shown (auto-enabled by min_entries:: filter). \
     Expected to see session details like '- session-id'. stdout: {stdout}"
  );
}
