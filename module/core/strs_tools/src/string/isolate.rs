...
pub mod private
{
    ...
    pub fn isolate( &self ) -> ( &'a str, Option<&'a str>, &'a str )
    {
      ...
      if self.left.0
      {
        ...
        else
        {
          ...
          {
            let delim_opt = if self.delimeter.0.is_empty() { None } else { Some( self.delimeter.0 ) };
  ...
  {
    IsolateOptions { left: Left( true ), ..IsolateOptions::default() }
  }
  ...
  {
    IsolateOptions { left: Left( false ), ..IsolateOptions::default() }
  }
}
