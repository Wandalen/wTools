...
> **Source document.** This file is a non-compliant combined source document predating the `docs/` doc entity structure. Its design concepts have been migrated to compliant doc instances under `docs/feature/`, `docs/api/`, `docs/algorithm/`, and `docs/invariant/`. Do not edit this file — edit the authoritative doc instances instead. Exception: the § Async-Ready Design section was NOT migrated (YAGNI — documents a hypothetical future that was never implemented).
