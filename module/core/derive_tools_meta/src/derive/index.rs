...
pub fn index(input: proc_macro::TokenStream) -> Result<proc_macro2::TokenStream> {
  ...
  let result = match parsed {
    StructLike::Struct(ref item) => generate_struct(item_name, &generics_impl, &generics_ty, &generics_where, &item.fields),
    StructLike::Enum(_) => todo!(),
    StructLike::Unit(_) => todo!(),
  }?;
...
fn generate_struct(
  ...
  match fields {
    syn::Fields::Named(fields) => generate_struct_named_fields(item_name, generics_impl, generics_ty, generics_where, fields),
    syn::Fields::Unnamed(fields) => generate_struct_tuple_fields(item_name, generics_impl, generics_ty, generics_where, fields),
...
fn generate_struct_tuple_fields(
  ...
  let match_arms = fields.iter().enumerate().map_result(|(i, field)| {
...
fn generate_struct_named_fields(
  ...
  let field_type = match fields.first() {
    Some(field) => &field.ty,
    None => return Err(syn::Error::new(fields.span(), "Empty type cannot be indexed")),
  };
  // Generate match arms for each field
  let match_arms = fields.iter().enumerate().map_result(|(i, field)| {
    let field_name = &field.ident;
