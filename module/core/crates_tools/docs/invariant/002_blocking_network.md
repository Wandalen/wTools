...
- **Out of Scope**: Archive storage (see `001_in_memory_storage.md`), API signatures (see `../api/`).
...
All network operations in `crates_tools` are synchronous and blocking. `download()` and `download_crates_io()` block the calling thread until the download completes (success) or fails (error). No async/await interface exists.

The HTTP client (`ureq`) is configured with:
- Read timeout: 5 seconds
- Write timeout: 5 seconds

After the timeout window the operation returns `Err( ureq::Error )` rather than hanging indefinitely. No automatic retry occurs; each call is a single attempt.
...
- **Thread blocking**: Calling `download*` from an async executor blocks the executor thread. Callers in async contexts must wrap the call in `tokio::task::spawn_blocking` or equivalent.
