...
The `FileSystem` trait defines three operations: write (file path + bytes → disk), read (file path → bytes), and create-directory-all (path → directory tree). Any type implementing this trait can back template generation. The abstraction boundary enables unit testing via `MemoryFileSystem` without filesystem side effects.
