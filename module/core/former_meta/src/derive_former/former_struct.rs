...
specific needs of the broader forming context. It mandates the implementation of `former::FormingEnd`.
  ...
  // Now that macro_tools::generic_params::decompose is fixed, we can use the output directly
  ...
  let standalone_constructor_code = if struct_attrs.standalone_constructors.value(false) {
    ...
    let (return_type, constructor_body) = if all_fields_are_args {
      ...
      let former_return_type = if has_non_lifetime_generics {
        quote! {
          #former < #struct_generics_impl_without_lifetimes, #former_definition< #former_definition_args > >
        }
      } else {
        quote! {
