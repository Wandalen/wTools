...
use super::ident;
use syn::{ DeriveInput, Variant, Visibility, Generics, WhereClause, parse_quote };
use super::{ ItemAttributes, FieldAttributes, EnumVariantFieldInfo };
...
(
  ...
  _end_impls : &mut Vec<TokenStream>,
...
Result< () >
{
  ...
  // Check for #[subform_scalar] attribute - not allowed on zero-field struct variants
  if variant_attrs.subform_scalar.is_some()
  {
    return Err( syn::Error::new_spanned( variant, "#[subform_scalar] is not allowed on zero-field struct variants" ) );
  }

  // Check for #[scalar] attribute - required for zero-field struct variants
  if variant_attrs.scalar.is_none()
  {
      return Err( syn::Error::new_spanned( variant, "#[scalar] is required for zero-field struct variants" ) );
  }
