...
use macro_tools::{ Result, quote::quote, syn, diag }; // Removed unused TokenStream
...
Result< () >
{
  ...
  {
      return Err( syn::Error::new_spanned( ctx.variant, "#[scalar] is required for zero-field struct variants" ) );
  }
  ...
  ctx.methods.push( method.clone() ); // Add to methods via context
  ...
  {
      ctx.standalone_constructors.push( method ); // Add to standalone constructors via context
  }
