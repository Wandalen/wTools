...
pub(crate) fn handle(ctx: &mut EnumVariantHandlerContext<'_>) -> Result<TokenStream> {
  ...
  let field = ctx.variant_field_info.first().ok_or_else(|| {
    syn::Error::new_spanned(
      ctx.variant,
      "Tuple variant with subform behavior must have exactly one field.",
    )
  })?;
  let field_ty = &field.ty;

  let type_path_str = quote! { #field_ty }.to_string().replace(' ', "");
  let is_phantom_data_field =
    type_path_str.starts_with("core::marker::PhantomData") || type_path_str.starts_with("std::marker::PhantomData");
