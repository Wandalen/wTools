...
(
  ...
  variant_attrs : &'a FieldAttributes,
  variant_field_info : &'a Vec<EnumVariantFieldInfo>,
...
) -> Result< () >
{
  ...
  let fields = match &variant.fields { Fields::Unnamed( f ) => f, _ => unreachable!() };
  ...
  match field_count
  {
    1 =>
    {
      ...
      else // Default or explicit subform_scalar -> Subformer
      {
        ...
        let phantom_field_type = macro_tools::phantom::tuple( &generics.params );
