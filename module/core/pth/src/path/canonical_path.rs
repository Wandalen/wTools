...
mod private
{
  ...
  /// Type alias for `NormalizedPath`.
  ///
  /// `CanonicalPath` and `NativePath` were functionally identical types that both represented
  ...
  /// # Rationale
  ///
  /// The anticipated platform-specific differences between `CanonicalPath` and `NativePath`
  ...
  pub type CanonicalPath = NormalizedPath;
}
