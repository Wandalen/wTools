...
#### 3.1. Setter Method Implementation

All generated scalar setter methods follow this signature pattern to enable move semantics and ergonomic APIs:

```rust
pub fn field_name( mut self, value : impl Into< FieldType > ) -> Self
{
  self.storage.field_name = Some( value.into() );
  self
}
```

**Behavioral Guarantees:**
- **Move Semantics:** Values are consumed via `.into()`, enabling zero-copy transfers when possible
- **No Defensive Cloning:** The generated code never performs unnecessary clones
- **Type Conversion:** Using `impl Into<T>` provides ergonomic APIs (e.g., accepts both `String` and `&str`)
- **Builder Chaining:** Methods return `Self` to enable fluent method chaining

**Rationale:** This pattern ensures optimal runtime performance while maintaining an ergonomic API. The use of `impl Into<T>` eliminates the need for users to explicitly convert types while avoiding runtime overhead from defensive cloning.
...
5.  **Performance Validation:** Execute benchmark suite to verify performance characteristics remain within specified bounds (see Section 9).

### 9. Performance Characteristics

This section documents the expected performance characteristics of code generated by the `former` macro. These characteristics serve as both implementation requirements and user expectations.

#### 9.1. Memory Allocation Patterns

**Setter Methods:**
- Setter methods consume values via move semantics (using `impl Into<T>`)
- No defensive cloning is performed in generated setter code
- Builder fields are stored as `Option<T>` in the `...FormerStorage` struct
- Memory allocation occurs only when necessary during the `.form()` operation

**Rationale:** Move semantics minimize memory allocations and copies, improving both runtime performance and memory efficiency.

#### 9.2. Compile Time Characteristics

**Macro Expansion:**
- Macro expansion overhead scales linearly with field count
- Expected overhead: ~200ms for complex structs (20+ fields) on typical development hardware
- Generated code size is minimized through helper function extraction

**Compilation Impact:**
- Incremental compilation is preserved - changes to user code don't re-expand the macro unnecessarily
- Generated code follows patterns that enable efficient incremental compilation
- No significant impact on clean build times for projects with moderate Former usage (< 100 derivations)

**Rationale:** Efficient macro implementation ensures that using Former doesn't significantly degrade developer experience during iterative development.

#### 9.3. Runtime Performance

**Builder Operations:**
- Builder creation: Constant time O(1) - allocates storage struct
- Setter calls: Constant time O(1) - stores value in Option field
- Form operation: Linear time O(n) where n is field count - unwraps Options and constructs final struct
- Builder chaining: Zero-cost abstraction - inlined by compiler

**Memory Efficiency:**
- Builder instances: Approximately same size as the target struct plus Option overhead
- No heap allocations for scalar fields during building (values moved directly into storage)
- Collection builders may allocate during construction as necessary for the collection type

**Rationale:** The generated code should impose minimal runtime overhead compared to manual builder implementations, making Former suitable for performance-sensitive code.

#### 9.4. Optimization Guarantees

The following optimizations are guaranteed by the generated code:

1. **Inlining:** All setter methods are marked `#[inline]` (when appropriate) to enable compiler optimizations
2. **Move Semantics:** Values are moved, not copied, whenever possible
3. **Zero-Cost Abstractions:** Builder patterns compile down to equivalent performance as manual initialization
4. **No Hidden Allocations:** All allocations are explicit in the type signatures and visible to users

**Performance Anti-Patterns to Avoid:**
- ❌ Defensive cloning in setter methods
- ❌ Unnecessary intermediate allocations
- ❌ Runtime type checks or reflection
- ❌ Hidden computational costs in generated code
