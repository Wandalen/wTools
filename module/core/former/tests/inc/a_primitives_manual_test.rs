  ...
  __FormerEnd: former::ToSuperFormer<Struct1, __FormerContext>,
{
  ...
  fn form( mut self ) -> Struct1
  {
    ...
    {
      self.container.string_1.take().unwrap()
    }
    else
    {
    ...
    let int_optional_1 = if self.container.int_optional_1.is_some()
    {
      Some( self.container.int_optional_1.take().unwrap() )
    }
    else
    {
      None
    ...
    let string_optional_1 = if self.container.string_optional_1.is_some()
    {
      Some( self.container.string_optional_1.take().unwrap() )
    }
    else
    {
      None
  ...
  {
    debug_assert!( self.container.int_1.is_none() );
    self.container.int_1 = Some( src.into() );
    self
  }
  ...
  {
    debug_assert!( self.container.string_1.is_none() );
    self.container.string_1 = Some( src.into() );
    self
  }
  ...
  where Src : core::convert::Into< String >
  {
    debug_assert!( self.container.string_optional_1.is_none() );
    self.container.string_optional_1 = Some( src.into() );
    self
  }
