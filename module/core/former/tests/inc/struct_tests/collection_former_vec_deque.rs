...
fn add() {
  ...
  let got: VecDeque<String> = the_module::CollectionFormer::<
    String,
    former::VecDequeDefinition<String, (), VecDeque<String>, the_module::ReturnStorage>,
  >::new(former::ReturnStorage)
  .add("a")
  .add("b")
  .form();
  let exp = collection_tools::deque!["a".to_string(), "b".to_string(),];
  a_id!(got, exp);

  // explicit with VecDequeFormer

  let got: VecDeque<String> =
    the_module::VecDequeFormer::<String, (), VecDeque<String>, the_module::ReturnStorage>::new(former::ReturnStorage)
      .add("a")
      .add("b")
      .form();
  let exp = collection_tools::deque!["a".to_string(), "b".to_string(),];
  ...
  let got: VecDeque<String> = the_module::VecDequeFormer::begin(
    Some(collection_tools::deque!["a".to_string()]),
    Some(()),
  ...
  let got: VecDeque<String> = VecDeque::former().add("a").add("b").form();
  let exp = collection_tools::deque!["a".to_string(), "b".to_string(),];
  a_id!(got, exp);
...
fn entity_to() {
  ...
  let got = <VecDeque<i32> as former::EntityToFormer<
    former::VecDequeDefinition<i32, (), VecDeque<i32>, former::ReturnStorage>,
  ...
  let exp = <VecDeque<i32> as former::EntityToFormer<
    former::VecDequeDefinition<i32, (), VecDeque<i32>, former::ReturnStorage>,
  >>::Former::new(former::ReturnStorage)
  .form();
  a_id!(got, exp);
