    ...
    let on_end = |substorage: ChildFormerStorage, super_former: core::option::Option<Self>| -> Self {
      ...
      if let Some(ref mut children) = super_former.storage.children {
        former::CollectionAdd::add(
          children,
          <<Vec<Child> as former::Collection>::Val as former::ValToEntry<Vec<Child>>>::val_to_entry(
...
impl<Types2, Definition> former::FormingEnd<Types2> for ParentSubformEntryChildrenEnd<Definition>
where
