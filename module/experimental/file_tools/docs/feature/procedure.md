...
- **Trigger**: A committed task exists for a new `file_tools` user-facing capability (status 🔄 Planned or higher).
- **Audience**: Contributors adding, updating, or deprecating feature doc instances.

### Procedure: Create Instance

1. Confirm the feature belongs here — verify it fits **In Scope** in `readme.md`. API signatures go in `../api/`; design decisions go in `../pattern/`; correctness properties go in `../invariant/`. Apply the YAGNI gate: only create when a committed task exists.
