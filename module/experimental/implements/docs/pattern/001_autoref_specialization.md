...
- **Purpose**: Simulate trait-conditional branching on the stable compiler toolchain without unstable specialization, proc-macros, or runtime overhead.
...
Determining at compile time whether a type satisfies a given trait bound, with the result exposed as a runtime boolean, requires conditional code generation. The stable compiler toolchain provides no direct mechanism for this: trait specialization requires unstable compiler features, const trait evaluation requires unstable compiler features, and procedural macros add an unwanted dependency. A solution must work on the stable toolchain with zero external dependencies.

### Solution

Two traits are defined within a local scope — one with an unconditional implementation (always returns false) and one with a conditional implementation (returns true only when the inspected type satisfies the required bound). The unconditional implementation is placed on a reference-wrapped phantom type; the conditional implementation is placed on the phantom type directly. A helper constructs the phantom type from a shared reference to the inspected value. Because the language's method resolution prefers less-indirected receivers, the conditional implementation is selected when the bound is satisfied and the unconditional implementation is selected otherwise. The result is a compile-time constant emitted as a runtime boolean.
...
- The stable compiler toolchain is required (no nightly-only features).
