...
| IN-05 | temporary_pattern_beats_permanent_same_depth | invariant | ⏳ |
...
### IN-05: temporary pattern beats permanent pattern at the same directory depth

- **Given:** Both `-{app}/config.yaml` (temporary) and `.{app}/config.yaml` (permanent) exist in the **same** directory; both contain a value for `"param1"` with different values
- **When:** `resolve_config_value("param1", &{})` is called with `file_ops` feature
- **Then:** The temporary (`-{app}`) value is returned — within the same depth, temporary pattern (gitignored) takes priority over permanent pattern (version-controlled)
- **Tests:** `tests/dual_pattern_tests.rs::test_temporary_beats_permanent_same_depth`
