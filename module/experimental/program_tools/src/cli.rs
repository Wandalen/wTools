...
mod private
{
  use crate ::{ run, run_project, RunOptions };
  use crate ::program ::Plan;
  use clap ::{ Parser, Subcommand };
  use std ::path ::PathBuf;

  #[ derive( Parser ) ]
  #[ command( about = "Run Rust files and projects as scripts." ) ]
  struct Cli
  {
    #[ command( subcommand ) ]
    command : Commands,
  ...
  ///
  /// Shared entry point for both the `program_tools` and `pt` binaries.
  /// The binary name shown in help output is inferred from `argv[0]` by clap.
  pub fn run_cli()
  {
    let cli = Cli::parse();
