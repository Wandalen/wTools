...
- **Responsibility**: Document the planned 5-phase algorithm that would replace the current implementation in `action/publish.rs` and `tool/graph.rs`.
...
**Phase 2 — Dependency Staleness Detection**: For each package P not in Phase 1 output, check all dependencies D. Mark P stale if D is being published OR if the workspace version of D does not satisfy P's requirement for D. Current implementation uses `BeingPublished` state but ignores version compatibility.
...
|------|------|----------------|
| doc | `001_workspace_management.md` | Current implementation behavior and known bugs |
| doc | `../../task/backlog/001_wrong_publish_set_graphs_tools.md` | Root cause analysis of wrong publish set (Won't Fix) |
